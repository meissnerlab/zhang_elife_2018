...
'''
biseqMethCalling.py
(C) Christoph Bock, Natalie Jaeger, Fabian Mueller, Michael Ziller
version: 2012-08-22
...
'main'        : basically levels A and B from the structure above
                prepares the parameters and invokes manageProcesses() to divide up the genome into subtasks (ready to be processes in a 'region' tasked
...
def strRevComp(s):
    ...
    '''
    return(Bio.Seq.reverse_complement(Bio.Seq.Seq(s)).tostring())
...
#                readSequence = readSequence[::-1]
        ...
        if options.mismatchTruncate > 0:
            ...
            mmPos = self.matchBitArray[self.seqMotifLength:(self.seqMotifLength+rl)].search(bitarray('0'))
    ...
    def setBluntEndRepairNeglectedBasesWGBS(self,nClip):
        '''
        set the bases not to be analyzed that correspont to blunt end fill up
        those sites do not contain any information, as they are filled in with methylated Cs
        Do not repair end of fragments since we can assume that the fragment length is greater than the read length
        Assume orientation in PE dataset is 5->3 for read 1 and 3->5 for read 2. That means for read 1 the first bases are clipped, whereas
        for read 2 the last bases are being clipped
        '''
        if self.account4beingRead2:
            #self.excludedBases[:nClip] = True
            self.excludedBases[(self.readlength-nClip):self.readlength]=True
            #self.excludedBases[(self.fragmentLength-restrSite.fillInsStart):] = True #if the index is too large nothing happens
        else:
            self.excludedBases[:nClip] = True
            #self.excludedBases[(self.fragmentLength-restrSite.fillInsEnd):] = True #if the index is too large nothing happens 
...
def lookupSequence(chromFile, chromstart, chromend, offset):
    ...
    '''
    # retrieve genomic sequence
    pos = chromstart / 80 * 81 + chromstart % 80
    length = (chromend - chromstart + 80) / 80 * 81
    chromFile.seek(offset+pos,0)
    seq = chromFile.read(length)
    seq = seq.replace("\n","")
    seq = seq[0:(chromend-chromstart)]
...
class SpikeInControls:
    ...
    def __init__(self,filename,motifLen):
        ...
        for record in Bio.SeqIO.parse(f, "fasta"):
            ...
            countsFW = {}
            countsFWqual = {}
            for i in curSeqFWbitarrays["C"].search(bitarray('1')): #go through all C positions
                countsFW[i] = {"conv":0,"total":0}
                countsFWqual[i] = {"conv":0,"total":0}
            countsRC = {}
            countsRCqual = {}
            for i in curSeqRCbitarrays["C"].search(bitarray('1')): #go through all C positions
...
def sequenceDataProcessing(genomicRegion, startPos, endPos, statsPickledFile, outfileNames, strands, seqMotif, fragmentsInfo, chromSizeHash,chromFileHandler,restrSite):
    ...
    print "Processing genomic region "+genomicRegion +' '+ str(startPos)+' '+ str(endPos)
    sys.stdout.flush()
...
#    tempReadUnit = bsAlignedReadCollection() # container storing the reads for writing to files later
    ...
    print "number of reads: " + str(len(pysamReadList))
    sys.stdout.flush()
...
#DEBUG
    ...
    for readAndFileIndex in pysamReadList:
        ...
        sys.stdout.flush()
        if count % 100000 == 0:
            print "Processing " + genomicRegion + ":" + str(startPos) + " to " + str(endPos) + " record "+str(count)
...
#                print read.getDebugString()
        ...
        elif options.bluntEndRepair>0 and options.pairedEnd :
            read.setBluntEndRepairNeglectedBasesWGBS(options.bluntEndRepair)
...
#            tempReadUnit.addRead(read)
    ...
    print "[" + str(time.strftime("%Y-%m-%d %H:%M:%S")) + "] Region " + genomicRegion + " [" + str(startPos) + "," + str(endPos) + "): " + str(unalignedCount) + " of " + str(count) + " reads unaligned, " + str(countDiscardedAlignments) + " discarded"
    print "[" + str(time.strftime("%Y-%m-%d %H:%M:%S")) + "] Region " + genomicRegion + " [" + str(startPos) + "," + str(endPos) + "): updating statistics..." 
    sys.stdout.flush()
    ...
    print "[" + str(time.strftime("%Y-%m-%d %H:%M:%S")) + "] Region " + genomicRegion + " [" + str(startPos) + "," + str(endPos) + "): creating subprocess output..." 
    sys.stdout.flush()
...
#        print str(tempReadUnit.getNumReads()) + ' of ' + str(len(pysamReadList)) +' reads processed for Chromosome ' + genomicRegion + "[" + str(startPos) + "," + str(endPos) + ')'
        print str(processedReads) + ' of ' + str(len(pysamReadList)) +' reads processed for Chromosome ' + genomicRegion + "[" + str(startPos) + "," + str(endPos) + ')'
...
#        tempReadUnit.writeReadOutFile(seqMotif,outfileNames['readOutfile'])
        ...
        print "sorting reads in read output file: Reads |"+genomicRegion + "[" + str(startPos) + "," + str(endPos) + ')'+"| ..."
        sys.stdout.flush()
        readFileH.sort()
        ...
        while nextChromNo <= len(chromSortList):
            ...
            if (processUnit == None or chrom == None or curChromPos >= chromSizeHash[chrom]):
                ...
                if nextChromNo < len(chromSortList):
                    ...
                    print "estimated number of chromUnits for " + chrom + ": " + str(numChromUnits)
                    processUnit = 0
                    curChromPos = 0
                else:
...
#                processUnitEnd = readfileCollection.findNextGap(windowBoundary2try,chrom,chromSizeHash[chrom])
                ...
                if runprocess and overlapsIncludedRegions:
                    ...
                    for seqMotif in seqMotifs:
                        ...
                        numProcesses += 1
                        print "-------------------------------------------------------------------------------------------------"
                        print "Initializing process at " + time.strftime("%Y-%m-%d %H:%M:%S") + ": "+chrom+" "+str(processUnit)+" window: [" + str(processUnitStart) + "," + str(processUnitEnd) + ")"
...
def performAnalysis_region(options_l):
    ...
    print "-------------------------------------------------------------------"
    print "REGION process finished at "
    print "Local time: " + time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime())
...
def performAnalysis_wrapup(options_l):
    ...
    if len(missingPKLfiles) > 0:
        if (options_l.ignoreMissingFiles):
            print "ERROR: "+str(len(missingPKLfiles))+" Region picklefile(s) missing. Proceeding anyways"
        else:
...
#            os.system(callStringErrs)
    ...
    if options.bigBedFormat:
        ...
        chromSizesFile = options.outGenome+'.chrom.sizes'
        if not os.path.exists(options.outGenome+'.chrom.sizes'): # check again with above code for redundancy
            chromSizesFileCmd = toolsPath + 'fetchChromSizes ' +options.outGenome+' > '+options.outGenome+'.chrom.sizes'
            sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    parser.add_option('--bluntEndRepair',action='store',type='int',dest='bluntEndRepair',help='Specify the number of bases to ignore at the beginning of each read due to blunt end repair. Makes only sense for paired end sequencing. (default=0, no blunt end repair)', default=0)
...
#    parser.add_option('--customSwitch',action='store_true',dest='customSwitch',help='DO NOT USE UNLESS YOU HAVE A CLUE WHAT IT ACTUALLY DOES AT THE MOMENT. A variable switch that enables a certain debugging option of interest. depending on script status.',default=False)
    ...
    parser.add_option('--ignoreMissingFiles',action='store_true',dest='ignoreMissingFiles',help='Ignore missing pickle files in wrapup',default=False)
